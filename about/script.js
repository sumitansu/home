...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ============================================
  // Mobile tap toggle
  if (window.innerWidth < 640 && socialBox) {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (socialBox && !socialBox.contains(e.target)) {
      socialBox.classList.remove("open");
    }
  });
  ...
  const glowElements = document.querySelectorAll(
    ".nav-box, .social-box, .social-expanded, .menu-item, .nav-icon-btn, .social-link, .music-ctrl-btn, .volume-wrapper, .about-card",
  );
  ...
  // ============================================
  // RANDOM "WHO AM I" CONTENT
  // ============================================
  const whoAmIVariants = [
    {
      title: "Sumit | DoRaemon",
      body: "I’m a 16-year-old developer and gamer. I spend most of my time in-game, but I’m always working on coding projects when I'm away from the keyboard. I’m a pretty big extrovert, so don’t hesitate to reach out—I’m always down to chat or collaborate.",
    },
    {
      title: "Sumit",
      body: "16 | Gamer & Coder.<br>If I'm not gaming, I'm probably messing around with some code. I love meeting new people, so my DMs are always open. Say hi!",
    },
    {
      title: "Hi, I’m Sumit (aka DoRaemon).",
      body: "I’m a 16-year-old with a massive interest in gaming and software development. I switch between the two depending on the day, but I’m always looking to learn something new. I'm definitely an extrovert, so feel free to message me—I'll get back to you as soon as I can.",
    },
  ];

  const cardWhoTitle = document.querySelector("#cardWho .card-title");
  const cardWhoBody = document.querySelector("#cardWho .card-body p");

  if (cardWhoTitle && cardWhoBody) {
    let variantIndex = sessionStorage.getItem("whoAmIVariant");

    if (variantIndex === null) {
      variantIndex = Math.floor(Math.random() * whoAmIVariants.length);
      sessionStorage.setItem("whoAmIVariant", variantIndex);
    } else {
      variantIndex = parseInt(variantIndex, 10);
    }

    const selectedVariant = whoAmIVariants[variantIndex];

    cardWhoTitle.textContent = selectedVariant.title;

    if (selectedVariant.body.includes("<")) {
      cardWhoBody.innerHTML = selectedVariant.body;
    } else {
      cardWhoBody.textContent = selectedVariant.body;
    }
  }
  ...
  const interestContent = {
    gaming: "Currently most played is Roblox",
    coding: "HTML/CSS/JS and PY",
    editing: "Video editing in Capcut",
    "learning-editing": `
      <div class="progress-label"><span>Editing</span><span>40%</span></div>
      <div class="progress-container"><div class="progress-fill" style="width: 40%"></div></div>
    `,
    "learning-coding": `
      <div class="progress-label"><span>Coding</span><span>65%</span></div>
      <div class="progress-container"><div class="progress-fill" style="width: 65%"></div></div>
    `,
    exploiting: "Hacking in Roblox just for fun",
  };
  ...
  interestItems.forEach((item) => {
    ...
    item.addEventListener("mousemove", (e) => {
      // Offset tooltip slightly to the right/bottom of cursor
      const offsetX = 15;
      const offsetY = 15;
      ...
      // Boundary checks can be added here if needed to prevent off-screen
